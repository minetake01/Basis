...
{
    public class BasisLuauVideoPlayer : MonoBehaviour
    {
        ...
        public string url
        {
            ...
            set
            {
                ...
                {
                    BasisDebug.LogError($"[BasisLuauVideoPlayer] Failed to parse URL: {e}", BasisDebug.LogTag.Shims);
                }

                if (BasisTrustedUrls.IsTrusted(url))
                {
                    BasisDebug.Log($"[BasisLuauVideoPlayer] Auto-accepting trusted URL \"{url}\"", BasisDebug.LogTag.Shims);
                ...
                BasisDebug.Log($"[BasisLuauVideoPlayer] Requesting URL \"{url}\"", BasisDebug.LogTag.Shims);
                ...
                BasisMenuURLPromptPanel.CreateNew(
                    ...
                    response =>
                    {
                        BasisDebug.Log($"[BasisLuauVideoPlayer] URL Prompt Result: {(response.Accepted ? "Accepted" : "Declined")} {(response.RememberChoice ? "Remembered" : "")} {response.Scope}", BasisDebug.LogTag.Shims);
        ...
        // Delegates
        //
        public delegate void ErrorEventHandlerShim(BasisLuauVideoPlayer source, string message);
        public delegate void EventHandlerShim(BasisLuauVideoPlayer source);
        public delegate void FrameReadyEventHandlerShim(BasisLuauVideoPlayer source, long frameIndex);
        public delegate void TimeEventHandlerShim(BasisLuauVideoPlayer source, double seconds);
        //
        // Event Invokers
        //
        ...
        {
            BasisDebug.Log($"[BasisLuauVideoPlayer] Setting URL to \"{url}\"", BasisDebug.LogTag.Shims);
            videoPlayer.url = url;
        ...
        private IEnumerator ExpirePendingUrlRequest(int requestId)
        {
            ...
            BasisDebug.Log($"[BasisLuauVideoPlayer] Timed out waiting for approval: {pendingConfirmedUrl}", BasisDebug.LogTag.Shims);
            AutoDenyPendingUrlRequest();
        }
